...
import { getChartsPersonData } from '../api/ECharts.js'
...
const resizeMainContainer = () => {
    // 获取当前窗口的宽度和高度
    mainContainer.style.width = window.innerWidth*0.20 +'px';
    mainContainer.style.height = window.innerHeight*0.30+'px';
}

const loadData = () => { 
    getChartsPersonData().then(res => {
...
onMounted (()=> {
    loadData()
    mainContainer = document.getElementById('mainperson')
})
</script>